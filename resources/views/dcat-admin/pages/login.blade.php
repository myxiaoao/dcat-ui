            ...
            <div class="card-body login-card-body shadow-100">
                ...
                <form id="login-form" method="POST" action="{{ admin_url('auth/login') }}">
                    ...
                    <fieldset class="form-label-group form-group position-relative has-icon-left">
                        <input
                            type="text"
                            class="form-control {{ $errors->has('google2fa_code') ? 'is-invalid' : '' }}"
                            name="google2fa_code"
                            placeholder="{{ trans('admin.google2fa_code') }}"
                            value="{{ old('google2fa_code') }}"
                            required
                            autofocus
                        >
                        ...
                        <label for="google2fa_code">{{ trans('admin.google2fa_code') }}</label>

                        <div class="help-block with-errors"></div>
                        @if($errors->has('google2fa_code'))
                            <span class="invalid-feedback text-danger" role="alert">
                                            @foreach($errors->get('google2fa_code') as $message)
                                    <span class="control-label" for="inputError"><i class="feather icon-x-circle"></i> {{$message}}</span>
                                    <br>
                                @endforeach
                                        </span>
